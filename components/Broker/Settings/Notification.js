...
export default function Notification() {
  ...
  const getAllNotification = async () => {
    try {
      const resNotification = await axios.get(`/api/notification/all_notification/`)
      //console.log("All notifications",resNotification);
      setSteps1(resNotification?.response?.data?.data)
    } catch (err) {
      //console.log(err);
      setSteps1(err?.response?.data?.data)
    }
  }

  //console.log("here i am",steps1)
  return (
    <div>
      <h5 className="text-lg md:text-xl lg:text-2xl font-bold ">Notifications</h5>
                  ...
                  >
                    <div className=" text-sm">
                      <div className="flex gap-x-4 flex-wrap justify-between w-full ">
                        <p className="font-bold lg:text-base text-sm">{step?.title}</p>
